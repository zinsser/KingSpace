...
bool CExcelCtrl::Convert(CString SheetOrSeparator)
{
	// Prepare file
	m_stempString = m_sFile;
	...
	{
		m_stempString += L".csv";
		CExcelCtrl tempSheet(m_stempString, SheetOrSeparator, false);
...
bool CExcelCtrl::DeleteSheet(CString SheetName)
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		// Delete sheet
...
void CExcelCtrl::GetExcelDriver()
{
	wchar_t szBuf[2001];
	WORD cbBufMax = 2000;
	WORD cbBufOut;
...
// Set Auto to false if want to force column to be used as header name
bool CExcelCtrl::ReadCell (CString &CellValue, CString column, long row, bool Auto)
{
	short columnIndex = CalculateColumnNumber(column, Auto);
	if (columnIndex == 0)
	{
...
// Default is read the next cell in next row.
bool CExcelCtrl::ReadCell (CString &CellValue, short column, long row)
{
	if (column == 0)
	{
		m_sLastError = L"Column cannot be zero\n";
...
// Set Auto to false if want to force column to be used as header name
bool CExcelCtrl::ReadColumn(CStringArray &ColumnValues, CString column, bool Auto)
{
	short columnIndex = CalculateColumnNumber(column, Auto);
	if (columnIndex == 0)
	{
...
bool CExcelCtrl::ReadColumn(CStringArray &ColumnValues, short column)
{
	if (column == 0)
	{
		m_sLastError = L"Column cannot be zero\n";
...
bool CExcelCtrl::ReadRow(CStringArray &RowValues, long row)
{
...
bool CExcelCtrl::ReplaceRows(CStringArray &NewRowValues, CStringArray &OldRowValues)
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
